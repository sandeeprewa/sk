package com.academics.school.pl.rest.global.error;

public class RestErrorMessage {
	
	public static final String FIRST_NAME_ABSENT = "firstname.invalid.message";
	public static final String VALIDATION_FAILURE = "invalid.input.message";
	public static final String ADMISSION_FAILURE = "admission.fail.message";
	public static final String ALREADY_REGISTERED = "registration.alreadyregistered.message";
	public static final String UNEDITABLE_FIELD = "uneditable.field.message";
	public static final String STUDENT_DOES_NOT_EXIST = "student.does.not.exist.message";
	public static final String INPUT_IS_INVALID = "input.invalid";
	
	
	public static final String ALREADY_ADMITTED = "Admission.alreadyadmitted.message";
	
}
